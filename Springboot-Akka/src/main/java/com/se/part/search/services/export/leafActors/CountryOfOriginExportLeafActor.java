...
@Component(Constants.COOS_STRATEGY)
public class CountryOfOriginExportLeafActor extends BOMExporterActor
{
	@Value("#{environment['export.countryOfOriginSection.url']}")
	private String url;
	...
	public void exportResult(ExportMessage message)
	{
		...
		try
		{
			...
			CountryOfOriginExportRequest requestBody = getRequestBody(parts);
			...
			if(response.getStatusCode() == HttpStatus.OK)
			{
				...
				if(result != null && !result.isEmpty())
				{
					...
					Map<String, List<FeatureNameValueDTO>> featureNameValueMap = getFeatureNameFeatureValueMap(jsonResponse, Constants.COOS_STRATEGY, configuredFeaturesName);
					appendNotFoundPartsV2(featureNameValueMap, parts);
					sendMessageToCategoryManager(getSender(), new CountryOfOriginExportLeafActor.CountryOfOriginExportResultMessage(Constants.COOS_STRATEGY, featureNameValueMap, requestID), getSelf());
				}
				else
				{
		...
		{
			log.error(e, "Error during exporting CountryOfOrigin");
			Map<String, List<FeatureNameValueDTO>> featureNameValueMap = new HashMap<>();
			returnEmptyMapV2(parts, featureNameValueMap);
			getSender().tell(new CountryOfOriginExportLeafActor.CountryOfOriginExportResultMessage(Constants.COOS_STRATEGY, featureNameValueMap, requestID), getSelf());
		}
		long end = System.currentTimeMillis() - start;
		logger.info("CountryOfOrigin Export has been finished in:{} ms", end);
	}

	private CountryOfOriginExportRequest getRequestBody(Collection<String> parts)
	{
		CountryOfOriginExportRequest requestBody = new CountryOfOriginExportLeafActor.CountryOfOriginExportRequest();
		requestBody.setComIds(parts);
		return requestBody;
	}

	public class CountryOfOriginExportRequest
	{
	...
	public static class CountryOfOriginExportResultMessage
	{
		private final String categoryName;
		private final Map<String, List<FeatureNameValueDTO>> response;
		private final String requestId;

		public CountryOfOriginExportResultMessage(String categoryName, Map<String, List<FeatureNameValueDTO>> response, String requestId)
		{
			super();
