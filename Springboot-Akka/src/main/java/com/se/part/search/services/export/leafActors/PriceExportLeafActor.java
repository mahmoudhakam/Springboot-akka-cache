...
@Component(Constants.PRICE_STRATEGY)
public class PriceExportLeafActor extends BOMExporterActor
{
	@Value("#{environment['export.priceSection.url']}")
	private String url;
	...
	public void exportResult(ExportMessage message)
	{
		...
		try
		{
			...
			PriceExportRequest requestBody = getRequestBody(parts);
			...
			if(response.getStatusCode() == HttpStatus.OK)
			{
				...
				if(result != null && !result.isEmpty())
				{
					...
					Map<String, List<FeatureNameValueDTO>> featureNameValueMap = getFeatureNameFeatureValueMap(jsonResponse, Constants.PRICE_STRATEGY, configuredFeaturesName);
					appendNotFoundPartsV2(featureNameValueMap, parts);
					sendMessageToCategoryManager(getSender(), new PriceExportLeafActor.PriceExportResultMessage(Constants.PRICE_STRATEGY, featureNameValueMap, requestID), getSelf());
				}
				else
				{
		...
		{
			log.error(e, "Error during exporting Price");
			Map<String, List<FeatureNameValueDTO>> featureNameValueMap = new HashMap<>();
			returnEmptyMapV2(parts, featureNameValueMap);
			getSender().tell(new PriceExportLeafActor.PriceExportResultMessage(Constants.PRICE_STRATEGY, featureNameValueMap, requestID), getSelf());
		}
		long end = System.currentTimeMillis() - start;
		logger.info("Price Export has been finished in:{} ms", end);
	}

	private PriceExportRequest getRequestBody(Collection<String> parts)
	{
		PriceExportRequest requestBody = new PriceExportLeafActor.PriceExportRequest();
		requestBody.setComIds(parts);
		return requestBody;
	}

	public class PriceExportRequest
	{
	...
	public static class PriceExportResultMessage
	{
		private final String categoryName;
		private final Map<String, List<FeatureNameValueDTO>> response;
		private final String requestId;

		public PriceExportResultMessage(String categoryName, Map<String, List<FeatureNameValueDTO>> response, String requestId)
		{
			super();
