...
@Component(Constants.CHINAROHS_STRATEGY)
public class ChinaROHSExportLeafActor extends BOMExporterActor
{
	@Value("#{environment['export.chinaROHSSection.url']}")
	private String url;
	...
	public void exportResult(ExportMessage message)
	{
		...
		try
		{
			...
			ChinaROHSExportRequest requestBody = getRequestBody(parts);
			...
			if(response.getStatusCode() == HttpStatus.OK)
			{
				...
				if(result != null && !result.isEmpty())
				{
					...
					Map<String, List<FeatureNameValueDTO>> featureNameValueMap = getFeatureNameFeatureValueMap(jsonResponse, Constants.CHINAROHS_STRATEGY, configuredFeaturesName);
					appendNotFoundPartsV2(featureNameValueMap, parts);
					sendMessageToCategoryManager(getSender(), new ChinaROHSExportLeafActor.ChinaROHSExportResultMessage(Constants.CHINAROHS_STRATEGY, featureNameValueMap, requestID), getSelf());
				}
				else
				{
		...
		{
			log.error(e, "Error during exporting ChinaROHS");
			Map<String, List<FeatureNameValueDTO>> featureNameValueMap = new HashMap<>();
			returnEmptyMapV2(parts, featureNameValueMap);
			getSender().tell(new ChinaROHSExportLeafActor.ChinaROHSExportResultMessage(Constants.CHINAROHS_STRATEGY, featureNameValueMap, requestID), getSelf());
		}
		long end = System.currentTimeMillis() - start;
		logger.info("ChinaROHS Export has been finished in:{} ms", end);
	}

	private ChinaROHSExportRequest getRequestBody(Collection<String> parts)
	{
		ChinaROHSExportRequest requestBody = new ChinaROHSExportLeafActor.ChinaROHSExportRequest();
		requestBody.setComIds(parts);
		return requestBody;
	}

	public class ChinaROHSExportRequest
	{
	...
	public static class ChinaROHSExportResultMessage
	{
		private final String categoryName;
		private final Map<String, List<FeatureNameValueDTO>> response;
		private final String requestId;

		public ChinaROHSExportResultMessage(String categoryName, Map<String, List<FeatureNameValueDTO>> response, String requestId)
		{
			super();
